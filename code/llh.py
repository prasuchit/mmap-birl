...
'''
NOTE: This file is the meat of the algorithm. This is where gradient calculation
and marginalization happens. There's an option to use parallel processing for 
marginalization but for small mdps it doesn't make much difference.
'''
...
def calcNegMarginalLogPost(w, trajs, mdp, options, problem):
    ...
    if np.isinf(post) or np.isinf(-post) or np.isnan(post):
        print(
            f'ERROR: prior: {prior}, llh: {llh}, eta: {options.eta}, w:{np.amin(w)} {np.amax(w)} \n')
        raise SystemExit(0)
    return post, grad
            ...
            for o in tqdm(range(len(occs))):
                ...
                for s in originalInfo.allOccNxtSts[o]:
                    for a in range(mdp.nActions):
                        ...
                        mresult.append(pool.apply_async(
                            calcLogLLH, (w, trajInfo, mdp, options)))
            ...
            for o in tqdm(range(len(occs))):
                for s in originalInfo.allOccNxtSts[o]:
                    for a in range(mdp.nActions):
                        ...
                        mresult.append(pool.apply_async(
                            calcLogLLH_obsv, (w, obsvsCopy, obs_prob, mdp, options)))
        ...
        for o in tqdm(range(len(occs))):
            for s in originalInfo.allOccNxtSts[o]:
                for a in range(mdp.nActions):
                    obsvsCopy[occs[o, 0], occs[o, 1], 0] = s
                    obsvsCopy[occs[o, 0], occs[o, 1], 1] = a
                    mllh, mgrad1 = calcLogLLH_obsv(
                        w, obsvsCopy, obs_prob, mdp, options)
                    llh += mllh
                    grad1 += mgrad1
        grad1 = np.reshape(grad1, (mdp.nFeatures, 1))
    else:
...
def calcLogPrior(w, options):
    if options.priorType == 'Gaussian':
        x = w - options.mu
        prior = np.sum(np.matmul(np.transpose(x), x) * -
                       1 / 2 * math.pow(options.sigmasq, 2))
...
def calcLogLLH_obsv(w, obsvs, obs_prob, mdp, options):
    ...
    for t in range(nTraj):
        ...
        t_grad = np.zeros(nF)
        h_theta_sum = 0
        tau_sa = generator.sampleTauTrajectories(
            mdp, obsvs[t], sampling_quantity, np.shape(obsvs)[1], None)
        for m in range(sampling_quantity):
            ...
            if obs_prob_prod > 0:
                ...
                if trans_prob_prod > 0:
                    ...
                    dh_theta_sum += c_tau * \
                        (calc_pi_sto_grad(tau_sa[m], pi_sto, nF, nA, dQ))
...
def calcGradQ(piL, mdp):
    ...
    if mdp.useSparse:
        ...
        dQ = np.linalg.lstsq(sparse.eye(nS * nA) -
                             (mdp.T * Epi), (mdp.F).todense())[0]
    else:
