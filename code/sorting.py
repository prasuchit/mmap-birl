...
def init(nOnionLoc, nEEFLoc, nPredict, nlistIDStatus, discount, useSparse, noise=0.05):
    ...
    for s in range(nS - 1):
        ...
        f = np.zeros((nF, 1))
        for a in range(nA):
            nextStates = utils3.findNxtStates(onionLoc, eefLoc, pred, listidstatus, a, numObjects)
            for nxtS in nextStates:
                ...
                # if s == nS - 2 and a == 0:
                #     print("Hey")
                if not (utils3.isValidState(onionLoc, eefLoc, pred, listidstatus, s, ns)):
                    ...
                    if a not in actidx:  # Invalid actions
                        if not (utils3.isValidNxtState(a, nxtS[0], nxtS[1], nxtS[2], nxtS[3])):
